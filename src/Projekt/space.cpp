...
Space::Space() {
  ...
  if (!shader) shader = ShaderPtr(new Shader{space_vertex, space_fragment});
  if (!texture) texture = TexturePtr(new Texture{"background.rgb", 800, 400});
